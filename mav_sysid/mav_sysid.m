...
clc
clear all
bag_name = 'sis50.bag';  %% so far sis25.bag is the best 
imu_topic = '/mavros/imu/data';
control_topic = '/m100/setpoint_raw/roll_pitch_yawrate_thrust';
...
bag = ros.Bag.parse(bag_name);
% bag.info

imu_data = readImu(bag, imu_topic,sys_id_start_time_s,sys_id_end_time_s);
attitude_cmd = readCommandRollPitchYawRateThrust(bag, control_topic,sys_id_start_time_s,sys_id_end_time_s);
%%
...
hold on;
plot(attitude_cmd.t, attitude_cmd.thrust/2, '--', 'linewidth', 2);
xlabel('time');
ylabel('???');
title('Thrust');
...
attitude_cmd.thrust_interp = interp1(attitude_cmd.t, attitude_cmd.thrust(1,:), imu_data.t);
